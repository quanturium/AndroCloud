...
import com.quanturium.androcloud.other.CustomMultiPartEntity;
import com.quanturium.androcloud.other.CustomMultiPartEntity.ProgressListener;
...
public class CloudAppItemsImpl extends CloudAppBase
{
	 ...
	 * {@inheritDoc}
	 * 
	 * @see com.cloudapp.api.CloudAppItems#createBookmark(java.lang.String, java.lang.String)
	 */
	public CloudAppItem createBookmark(String name, String url) throws CloudAppException
	{
		try
		{
			JSONObject json = createBody(new String[] { "name", "redirect_url" }, new String[] { name, url });
	 ...
	 */
	public List<CloudAppItem> createBookmarks(String[][] bookmarks) throws CloudAppException
	{
		try
		{
	 ...
	 * {@inheritDoc}
	 * 
	 * @see com.cloudapp.api.CloudAppItems#getItems(int, int, com.cloudapp.api.CloudAppItem.Type, boolean, java.lang.String)
	 */
	public List<CloudAppItem> getItems(int page, int perPage, CloudAppItem.Type type, boolean showDeleted, String source) throws CloudAppException
	{
		try
		{
			if (perPage < 5)
			...
			HttpGet req = new HttpGet(ITEMS_URL + "?" + Tools.implode("&", (String[]) params.toArray(new String[params.size()])));
		...
		catch (ClientProtocolException e)
		{
			...
			throw new CloudAppException(500, "Something went wrong trying to contact the CloudApp API", e);
		}
		catch (IOException e)
		{
			LOGGER.error("Something went wrong trying to contact the CloudApp API.", e);
	 ...
	 */
	public CloudAppItem upload(File file, TransfertTask task) throws CloudAppException, ParseException, InterruptedException
	{
		try
		{
			...
			if (!json.has("params"))
			{
				...
				throw new CloudAppException(500, "Missing params object from the CloudApp API.", null);
			}

			return uploadToAmazon(json, file, task);
	 ...
	 * @throws InterruptedException 
	 */
	private CloudAppItem uploadToAmazon(JSONObject json, File file, final TransfertTask task) throws JSONException, CloudAppException, ParseException, IOException, InterruptedException
	{
		JSONObject params = json.getJSONObject("params");

		final HttpPost uploadRequest = new HttpPost(json.getString("url"));
		...
		CustomMultiPartEntity entity = new CustomMultiPartEntity(new ProgressListener() {
			...
			public void transferred(long num)
			{
				if (System.currentTimeMillis() > time + 500)
				{
					...
					Log.i("uploaded", num + " bytes (" + (int) ((num / (float) totalSize) * 100) + ") %");
					task.doPublishProgress((int) ((num / (float) totalSize) * 100));
				}
		...
		{
			throw new InterruptedException("Upload cancel manually");
		}
	}
	...
	public CloudAppItem recover(CloudAppItem item) throws CloudAppException
	{
		try
		{
			JSONObject json = createBody(new String[] { "deleted_at" }, new Object[] { JSONObject.NULL });
	...
	public CloudAppItem setSecurity(CloudAppItem item, boolean is_private) throws CloudAppException
	{
		try
		{
			JSONObject json = createBody(new String[] { "private" }, new Object[] { is_private });
			json = (JSONObject) executePut(item.getHref(), json.toString(), 200);
			return new CloudAppItemImpl(json);
		}
		catch (JSONException e)
